...
export function register33333Routes(app: Express): void {
  // n8n Fire — publish approved content (namespaced — avoids /api/post/publish collision)
  app.post('/api/33333/n8n/publish', async (req, res) => {
  ...
  app.get('/api/33333/n8n/engagement/pending', async (req, res) => {
    if (!require33333Secret(req, res)) return;
  ...
  app.post('/api/33333/n8n/syndicate', async (req, res) => {
    if (!require33333Secret(req, res)) return;
...
/** @deprecated Use unified webhook in server/index.ts — kept for re-export compatibility */
export function registerStripeWebhook(_app: Express): void {
  // Unified at POST /api/webhooks/stripe in server/index.ts
}
